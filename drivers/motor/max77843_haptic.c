...
#include "max77843_haptic.h"
#include "../sensorhub/stm/ssp_motor.h"
#endif

#define TEST_MODE_TIME 10000
#if defined(CONFIG_MOTOR_DRV_MIN_TIME)
#define MOTOR_MIN_TIME 40
#endif
...
int setMotorCallback(int (*callback)(int state)){
	if(!callback){
		...
		pr_info("[VIB] sensor callback is Null !%s\n", __func__);
	}
	return 0;
}
#endif
...
#if defined(CONFIG_MOTOR_DRV_MIN_TIME)
		if(value < MOTOR_MIN_TIME)
			value += MOTOR_MIN_TIME;
#endif
...
static void haptic_work(struct work_struct *work)
{
	...
	if (hap_data->timeout > 0) {
		...
		{
			pr_info("[VIB] %s sensorCallback is null.start\n", __func__);
