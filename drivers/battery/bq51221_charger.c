...
#if defined(CONFIG_WIRELESS_CHARGER_BQ51221_COOLING_FAN)
int bq51221_set_fan(struct i2c_client *client, bool on)
{
	int data = 0;
	int ret = 0, i = 0;
	int retry_cnt = 0;
	...
	if (on)
		enable = BQ51221_FAN_ON_VALUE;
	else
		enable = BQ51221_FAN_OFF_VALUE;

	for(i=0; i < 3; i++) {
		/* send Cooling control */
		ret = bq51221_reg_write(client, BQ51221_REG_USER_HEADER, BQ51221_PPP_HEADER_0x28);
		ret = bq51221_reg_write(client, BQ51221_REG_PROP_PACKET_PAYLOAD, BQ51221_FAN_CTRL_CMD);
		ret = bq51221_reg_write(client, BQ51221_REG_PROP_PACKET_PAYLOAD2, enable);
		...
		if(data == BQ51221_PTK_ERR_NO_ERR) {
			pr_err("%s sent Cooling Control!\n",__func__);
			ret = 1;
		} else {
			pr_err("%s can not send Cooling Control! err pkt = 0x%x\n",__func__, data);
			ret = -1;
		}
...
#if defined(CONFIG_WIRELESS_CHARGER_BQ51221_COOLING_FAN)
	bq51221_set_fan(charger->client,0);
	msleep(500);
	bq51221_set_fan(charger->client,1);
#endif
}
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_ONLINE:
			if(val->intval == POWER_SUPPLY_TYPE_WIRELESS) {
				...
				wake_lock_timeout(&charger->wpc_wake_lock, HZ * 9);
