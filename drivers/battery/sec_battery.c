...
#else
	} else if ((battery->cable_type == POWER_SUPPLY_TYPE_WIRELESS) && battery->pdata->wpc_temp_check) {
		union power_supply_propval value;
		int wpc_high_temp_2nd = battery->pdata->wpc_high_temp;
		int wpc_high_temp_recovery_2nd = battery->pdata->wpc_high_temp_recovery;

		if (battery->siop_level >= 100 && (battery->pdata->wpc_high_temp_2nd > battery->pdata->wpc_high_temp)) { // LCD OFF
			wpc_high_temp_2nd = battery->pdata->wpc_high_temp_2nd;
			wpc_high_temp_recovery_2nd = battery->pdata->wpc_high_temp_recovery_2nd;
		} else if (battery->siop_level < 100 && (battery->pdata->wpc_lcd_on_high_temp > battery->pdata->wpc_high_temp)) { // LCD ON
			wpc_high_temp_2nd = battery->pdata->wpc_lcd_on_high_temp;
			wpc_high_temp_recovery_2nd = battery->pdata->wpc_lcd_on_high_temp_rec;
		} else if (battery->siop_level < 100)
			return;

		if ((battery->chg_limit == SEC_BATTERY_CHG_TEMP_NONE || battery->chg_limit == SEC_BATTERY_CHG_TEMP_HIGH_1ST) &&
				(battery->pdata->wpc_high_temp != wpc_high_temp_2nd) &&
				(battery->chg_temp > wpc_high_temp_2nd)) {
			battery->chg_limit = SEC_BATTERY_CHG_TEMP_HIGH_2ND;
			value.intval = battery->pdata->wpc_charging_limit_current_2nd;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_MAX, value);
			dev_info(battery->dev,"%s: WPC Chg current 2nd is reduced by Temp: %d, siop(%d)\n",
					__func__, battery->chg_temp, battery->siop_level);
		} else if ((battery->chg_limit == SEC_BATTERY_CHG_TEMP_NONE) &&
			...
			dev_info(battery->dev,"%s: WPC Chg current is reduced by Temp: %d\n",
					__func__, battery->chg_temp);
		} else if ((battery->chg_limit == SEC_BATTERY_CHG_TEMP_HIGH_2ND) &&
				(battery->pdata->wpc_high_temp != wpc_high_temp_2nd) &&
				(battery->chg_temp < wpc_high_temp_recovery_2nd)) {
			battery->chg_limit = SEC_BATTERY_CHG_TEMP_HIGH_1ST;
			value.intval = battery->pdata->wpc_charging_limit_current;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_MAX, value);
			dev_info(battery->dev,"%s: WPC Chg current 2nd is recovered by Temp: %d, siop(%d)\n",
					__func__, battery->chg_temp, battery->siop_level);
		} else if ((battery->chg_limit != SEC_BATTERY_CHG_TEMP_NONE) &&
			...
			dev_info(battery->dev,"%s: WPC Chg current is recovered by Temp: %d\n",
				...
				(battery->chg_temp < battery->pdata->wpc_high_temp) &&
				(battery->current_max < battery->pdata->charging_current[battery->cable_type].input_current_limit)) {
			value.intval = battery->pdata->charging_current
				[battery->cable_type].input_current_limit;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_MAX, value);
			dev_info(battery->dev,"%s: WPC Chg current is reset by Temp: %d, siop(%d)\n",
					__func__, battery->chg_temp, battery->siop_level);
		}
#endif
	} else if (battery->chg_limit != SEC_BATTERY_CHG_TEMP_NONE) {
	...
	if (battery->status == POWER_SUPPLY_STATUS_CHARGING ||
		...
		if ((cable_type == POWER_SUPPLY_TYPE_HV_MAINS) ||
			...
			if (battery->pdata->wpc_temp_check &&
				battery->pdata->wpc_high_temp != battery->pdata->wpc_high_temp_2nd &&
				battery->charging_passed_time < 600)
				//wpc_high_temp multi control model,flat time_to_full when start charge.
				value.intval = battery->pdata->wpc_charging_limit_current + 80;
		...
		struct sec_battery_info *battery)
{
	...
	if (pdata->wpc_temp_check) {
		...
		ret = of_property_read_u32(np, "battery,wpc_high_temp_2nd", &temp);
		pdata->wpc_high_temp_2nd = (int)temp;
		if (ret) {
			pr_info("%s : wpc_high_temp_2nd is Empty\n", __func__);
			pdata->wpc_high_temp_2nd = pdata->wpc_high_temp;
		}
		...
		ret = of_property_read_u32(np, "battery,wpc_high_temp_recovery_2nd", &temp);
		pdata->wpc_high_temp_recovery_2nd = (int)temp;
		if (ret) {
			pr_info("%s : wpc_high_temp_recovery_2nd is Empty\n", __func__);
			pdata->wpc_high_temp_recovery_2nd = pdata->wpc_high_temp_recovery;
		}
		...
		ret = of_property_read_u32(np, "battery,wpc_charging_limit_current_2nd",
				   &pdata->wpc_charging_limit_current_2nd);
		if (ret) {
			pr_info("%s : wpc_charging_limit_current_2nd is Empty\n", __func__);
			pdata->wpc_charging_limit_current_2nd = pdata->wpc_charging_limit_current;
		}
