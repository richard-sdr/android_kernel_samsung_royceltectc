				       ...
				       int input_current)
{
	...
	if (!input_current) {
		...
		wake_lock(&charger->wpc_curr_wake_lock);
		if (input_current < pre_current_max) {
			...
			for(i=0; i < quotient_wpc; i++)
			{
				...
				max77843_write_reg(charger->i2c,
						set_reg, reg_data);
				msleep(SLEEP_STEP);
				pr_info("%s [%d] pre_current_max = %d mA \n", __func__, i, pre_current_max);
			}
			pre_current_max -= remainder_wpc;
			pr_info("%s pre_current_max = %d mA \n", __func__, pre_current_max);
			quotient = pre_current_max / 20;
			reg_data = quotient;
			max77843_write_reg(charger->i2c,
					   set_reg, reg_data);
		...
		wake_unlock(&charger->wpc_curr_wake_lock);
	} else {
		...
		if ((charger->cable_type == POWER_SUPPLY_TYPE_BATTERY) &&
			...
			pre_current_max = 500;
			pr_info("%s [chg off] pre_current_max = %d mA \n", __func__, pre_current_max);
