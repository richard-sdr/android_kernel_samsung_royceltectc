...
void reset_proximity_vdd(struct ssp_data *data)
{
    int err =-1;
    err = proximity_regulator_onoff(data, false);
    if (err < 0) {
		pr_err("[SSP] %s regulator_off fail err = %d\n",
			__func__, err);
	}
    	mdelay(10);

	err = proximity_regulator_onoff(data, true);
	if (err < 0) {
		pr_err("[SSP] %s regulator_on fail err = %d\n",
			__func__, err);
	}
    mdelay(10);
}
