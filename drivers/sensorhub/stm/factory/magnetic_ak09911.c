...
#define ROYCE_OFFSET_FOR_DUAL_SCREEN   1200
#endif
...
static int check_data_spec(struct ssp_data *data, int sensortype)
{
	if ((data->buf[sensortype].x == 0) &&
		...
		|| (data->buf[sensortype].y > GM_DATA_SPEC_MAX + ROYCE_OFFSET_FOR_DUAL_SCREEN)
		|| (data->buf[sensortype].y < GM_DATA_SPEC_MIN - ROYCE_OFFSET_FOR_DUAL_SCREEN)
...
int set_pdc_matrix(struct ssp_data *data)
{
	...
	msg->length = sizeof(data->pdc_matrix[FOLDER_OPEN]);
	msg->options = AP2HUB_WRITE;
	msg->buffer = kzalloc(sizeof(data->pdc_matrix[FOLDER_OPEN]), GFP_KERNEL);
	if (msg->buffer == NULL) {
	...
	msg->free_buffer = 1;
#if 1
	if(data->flip_status == 0)
		memcpy(msg->buffer, data->pdc_matrix[FOLDER_OPEN], sizeof(data->pdc_matrix[FOLDER_OPEN]));
	else
		memcpy(msg->buffer, data->pdc_matrix[FOLDER_CLOSE], sizeof(data->pdc_matrix[FOLDER_CLOSE]));
#else
	...
	//pr_info("[SSP]: %s: finished : data->folder_state = %d \n", __func__, data->folder_state);
	pr_info("[SSP]: %s: finished : data->folder_state = %d data->flip_status = %d \n", __func__, data->folder_state, data->flip_status);
