...
#define FW_CHECKSUM_H		0xE8
#define FW_CHECKSUM_L		0xC2
			...
			struct abov_touchkey_platform_data *pdata)
{
	...
	pdata->vled = regulator_get(dev, pdata->led_supply_name);
	if (IS_ERR(pdata->vled)) {
		pdata->vled = NULL;
		dev_err(dev, "pdata->vled get error\n");
	}
	/* ROYCE */
	if((strncmp(pdata->fw_path, "abov/abov_tk_royce.fw", 21) == 0)){
		...
		case 0:
		case 1:
			ret = regulator_set_voltage(pdata->vled, 3000000, 3000000);
			break;
		case 2:
		case 3:
			ret = regulator_set_voltage(pdata->vled, 3100000, 3100000);
			break;
		default:
			ret = regulator_set_voltage(pdata->vled, 3000000, 3000000);
			break;
		...
		if (ret) {
			pdata->vled = NULL;
			dev_err(dev,"%s: Failed to set avdd: %d\n", __func__, ret);
		}
	}
			...
			struct abov_touchkey_platform_data *pdata)
{
	...
	retval = of_property_read_string(np, "abov,vled", &pdata->led_supply_name);
	if (retval)
		dev_err(dev, "unable to get name of vled, %d\n", retval);
...
static void abov_switching_tkey_work(struct work_struct *work)
{
	...
	if (info->flip_status != info->flip_status_current)
	{
		...
		if (info->flip_status_current == FLIP_CLOSE) {
			...
			if(info->pdata->vled)
			{
				if(regulator_is_enabled(info->pdata->vled))
					dev_info(&info->client->dev, "vdd led is on!\n");
				else {
					int ret = regulator_enable(info->pdata->vled);
					if (ret){
						dev_info(&info->client->dev, "vdd led set on fail(%d)!\n", ret);
					}
					dev_info(&info->client->dev, "vdd led set on!\n");
				}
			}
			...
			{
				if(!regulator_is_enabled(info->pdata->vled))
					dev_info(&info->client->dev, "vdd led is off!\n");
				else {
					int ret = regulator_disable(info->pdata->vled);
					if (ret){
						dev_info(&info->client->dev, "vdd led set on fail(%d)!\n", ret);
					}
					dev_info(&info->client->dev, "vdd led set off!\n");
				}
			}
...
static int abov_tk_input_open(struct input_dev *dev)
{
	...
	if(info->pdata->vled && info->flip_status_current == FLIP_CLOSE)
	{
		if(regulator_is_enabled(info->pdata->vled))
			dev_info(&info->client->dev, "vdd led is on!\n");
		else {
