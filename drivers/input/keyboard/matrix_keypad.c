...
#ifdef CONFIG_MATRIX_APWAKE_CALLING
#include <linux/wakelock.h>
struct wake_lock matrix_wakelock;
int call_enable_flag = 0;
EXPORT_SYMBOL(matrix_wakelock);
EXPORT_SYMBOL(call_enable_flag);
#endif
...
static void matrix_keypad_scan(struct work_struct *work)
{
...
#ifdef CONFIG_MATRIX_DOUBLE_CHECK
	uint32_t new_state2[MATRIX_MAX_COLS];
#endif
	int row, col, code;
	...
	memset(new_state2, 0, sizeof(new_state2));
	/* assert each column and read the row status out */
	for (col = 0; col < pdata->num_col_gpios; col++) {
	...
	for(col = 0; col < pdata->num_col_gpios; col++) {
		if(new_state[col] != new_state2[col])
		{
			pr_info("%s: 1st state=%x, 2nd state=%x!\n", __func__, new_state[col], new_state2[col]);
			goto exit_scan;
		}
	}
#endif
...
static irqreturn_t matrix_keypad_interrupt(int irq, void *id)
{
	...
	if(call_enable_flag)
	{
		pr_info("%s: lock esd wake lock!\n", __func__);
		wake_lock_timeout(&matrix_wakelock, 75*HZ);
	} else
		if(wake_lock_active(&matrix_wakelock))
			wake_unlock(&matrix_wakelock);
#endif
	 ...
	 * disable IRQs again.
	 */
	if (unlikely(keypad->scan_pending))
		goto out;
...
#ifdef CONFIG_MATRIX_APWAKE_CALLING
static ssize_t matrix_keypad_call_status_store(struct device *dev,
	struct device_attribute *attr, const char *buf,
	size_t size)
{
	int data;
	...
	if (data == 1) {
		call_enable_flag=1;
		pr_info("%s: call enable\n", __func__);
	}
	else{
		call_enable_flag=0;
		if(wake_lock_active(&matrix_wakelock))
			wake_unlock(&matrix_wakelock);
		pr_info("%s: call disable\n", __func__);
	}

	return size;
}

static ssize_t matrix_keypad_call_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", call_enable_flag);
}

static DEVICE_ATTR(call_status, S_IRUGO | S_IWUSR | S_IWGRP,
	matrix_keypad_call_status_show, matrix_keypad_call_status_store);
#endif
...
#ifdef CONFIG_MATRIX_APWAKE_CALLING
	wake_lock_init(&matrix_wakelock, WAKE_LOCK_SUSPEND, "matrix_wake_lock");
#endif
...
#ifdef CONFIG_MATRIX_APWAKE_CALLING
	err = device_create_file(sec_keypad, &dev_attr_call_status);
	if (err) {
		dev_err(&pdev->dev,"Failed to create device file in sysfs entries(%s)!\n",
				dev_attr_call_status.attr.name);
	}
#endif
...
#ifdef CONFIG_MATRIX_APWAKE_CALLING
	wake_lock_destroy(&matrix_wakelock);
#endif
err_free_mem:
