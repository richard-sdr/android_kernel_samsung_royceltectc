...
extern struct wake_lock matrix_wakelock;
extern int call_enable_flag;
#endif
...
#ifdef CONFIG_MATRIX_APWAKE_CALLING
	if(matrix_wakelock.ws.name!=NULL)
	{
		if(call_enable_flag){
...
void mms_set_tsp_info(struct mms_ts_info *mms_data)
{
	if (mms_data != NULL)
		...
		pr_err("%s [ERROR]: tsp info is null\n", __func__);
}
...
static void mms_switching_tsp_work(struct work_struct *work)
{
	...
	if (mms_data->flip_status != mms_data->flip_status_current)
	{
		...
		disable_irq(mms_data->client->irq);
		mms_data->flip_status = mms_data->flip_status_current;
		mms_power_control(mms_data, 0);
		...
		mms_clear_input(mms_data);
		msleep(800);
		mms_power_control(mms_data, 1);
		msleep(50);
		enable_irq(mms_data->client->irq);
				...
				unsigned long flip_cover, void *v)
{
	...
	schedule_work(&mms_data->switching_work.work);
...
#ifdef CONFIG_DUAL_TSP
	/*royce hw rev01 neednot update fw*/
	if(hw_rev>1){
#endif
