	...
	if (dev->num_inst == 1) {
		...
		if (ret) {
			...
			if (!dev->drm_fw_info.ofs) {
				...
				/* MEMPROT is needed instead of FW_LOADING in specific project */
				ret = exynos_smc(SMC_MEMPROT_MFCFW, 0, 0, 1);
#else
...
#ifdef CONFIG_BLOWFISH
		exynos_smc(SMC_MEMPROT_MFCFW, 0, 0, 0);
#endif
		dev->drm_fw_status = 0;
	}
#endif
			...
			if (s5p_mfc_wait_for_done_ctx(ctx,
				...
				if (dev->num_inst == 0) {
					...
					if (dev->is_support_smc) {
						...
						dev->is_support_smc = 0;
#ifdef CONFIG_BLOWFISH
						ret = exynos_smc(SMC_MEMPROT_MFCFW, 0, 0, 0);
						if (ret) {
							mfc_err_ctx("failed MFC DRM F/W unport(%d)\n",
									ret);
							mutex_unlock(&dev->mfc_mutex);
							return ret;
						}
