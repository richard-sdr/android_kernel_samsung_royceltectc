...
#if defined(CONFIG_MUIC_DET_JACK)
static unsigned int hw_rev = 0;
#endif
#if defined(CONFIG_SEC_FACTORY) && defined(CONFIG_MUIC_ROYCE)
static int is_15test_mode = 0;
#endif
...
#if defined(CONFIG_MUIC_ROYCE)
static ssize_t max77843_muic_set_jig_en(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
#if defined(CONFIG_SEC_FACTORY)
	...
	if (!strncasecmp(buf, "0", 1)) {
		is_15test_mode = 1;
		max77843_muic_update_reg(muic_data->i2c, MAX77843_MUIC_REG_CTRL3, 0x03, 0xFF, true);
		pr_err("%s:%s jig_on_n high\n", MUIC_DEV_NAME, __func__);
	} else if (!strncasecmp(buf, "1", 1)) {
		is_15test_mode = 0;
		max77843_muic_update_reg(muic_data->i2c, MAX77843_MUIC_REG_CTRL3, 0x0, 0xFF, true);
		pr_err("%s:%s jig_on_n auto detected\n", MUIC_DEV_NAME, __func__);
	} else {
...
#if defined(CONFIG_MUIC_ROYCE)
static DEVICE_ATTR(jig_en, 0664, NULL, max77843_muic_set_jig_en);
#endif
...
static void max77843_muic_earjack_detect_workfunc(struct work_struct *work)
{
	...
	/*OVP control of headset mic*/
	if(hw_rev < 2){
		if(muic_data->eardetected){
			dev_info(muic_data->dev, "%s: GPIO = %d,enable Gpio\n", __func__, muic_data->mic_en_gpio_rev01);
			gpio_direction_output(muic_data->mic_en_gpio_rev01, 1);
		}else {
			msleep(600); /*add 600ms to wait codec be off ,and then disable OVP gpio*/
			dev_info(muic_data->dev, "%s: GPIO = %d ,disable Gpio \n", __func__, muic_data->mic_en_gpio_rev01);
			gpio_direction_output(muic_data->mic_en_gpio_rev01, 0);
		}
	}else {
		struct regulator *regulator_avdd;
		int ret;

		regulator_avdd = regulator_get(NULL, muic_data->mic_en_gpio_rev02);
		if (IS_ERR(regulator_avdd)) {
			pr_err("%s:%s can't get ear mic vdd\n", MUIC_DEV_NAME, __func__);
		}

		if(muic_data->eardetected){
			ret = regulator_enable(regulator_avdd);
			if(ret < 0)
				pr_err("%s:%s can't get ear mic gpio\n", MUIC_DEV_NAME, __func__);
		}else {
			if (regulator_is_enabled(regulator_avdd)){
				msleep(600); /*add 600ms to wait codec be off ,and then disable OVP LDO*/
...
#if defined(CONFIG_SEC_FACTORY)
#if defined(CONFIG_MUIC_ROYCE)
				if (adc != ADC_JIG_UART_ON && is_15test_mode != 1) {
#else
...
#if defined(CONFIG_MUIC_DET_JACK)
	if(hw_rev < 2){
		muic_data->mic_en_gpio_rev01 = of_get_named_gpio(np_muic, "muic,usb-mic-en-gpio_rev01", 0);
	}else {
		of_property_read_string(np_muic, "muic,usb-mic-en-gpio_rev02", &muic_data->mic_en_gpio_rev02);
	}
#endif
...
static int __init get_hw_rev(char *arg)
{
	...
	pr_info("%s:%s hw_rev = %d\n", MUIC_DEV_NAME, __func__,hw_rev);
