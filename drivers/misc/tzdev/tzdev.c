		...
		int force, struct page **pages, unsigned long *verified_bitmap)
{
	...
	/* make migration */
	res = MIGRATE_PAGES(&pages_list, tzdev_alloc_kernel_page);
	if (res) {
		putback_lru_pages(&pages_list);
		return -EFAULT;
	}
...
static long tzdev_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TZIO_UPDATE_REE_TIME: {
		...
		dta = tzdev_smc_update_ree_time((int)ts.tv_sec, (int)ts.tv_nsec);
#else
		dta = tzdev_smc_update_ree_time(ts.tv_sec, ts.tv_nsec);
#endif
		if (put_user(dta, argp)) {
			ret = -EFAULT;
			break;
