...
#ifdef TIMA_LKM_BLOWFISH
#define TIMA_ON_BLOWFISH
#else
#define TIMA_ON_MC20
#endif
...
#ifdef TIMA_ON_BLOWFISH
//#include <mach/smc.h>

#define CUSTOM_SMC_FID		0xB2000202
#define SUNFUN_RSA_PKCS		90
...
static inline u32 smc_call(u32 cmd, u32 arg1, u32 arg2, u32 arg3)
{
  ...
  //  return _r0;
        ...
        printk("[DEBUG] SMC return : r0:0x%02x\t r1:0x%02x\t r2:0x%02x\t r3:0x%02x\n", cmd, arg1, arg2, arg3);
		printk("[DEBUG] SMC return : r0:0x%02x\t r1:0x%02x\t r2:0x%02x\t r3:0x%02x\n", _a0, _a1, _a2, _a3);

        return _r0;
}

#endif // TIMA_ON_BLOWFISH
...
{ 
	static struct lkm_info lkm_info1;
	static struct module_info module_info1;
	
	static unsigned char buf[1024];

	//static Elf_Ehdr * hdr;
	//static unsigned char * hdr;
	unsigned char *hdr_local;
	static struct page *pfn;
	static uint32_t *pfn_array;

	int ret = 0;
	int buf_len;
	//int len = info->len;
	unsigned long reg0 = CUSTOM_SMC_FID;
	unsigned long reg1 = SUNFUN_RSA_PKCS;
	...
	//Fill the module information in structure
	mutex_lock(&lkmauth_mutex);
	hdr_local = kmalloc(len,GFP_KERNEL);
	if(hdr_local == NULL)
	{
		printk("Not enough continuos memory \n");
		return RET_LKMAUTH_FAIL;
	}
	
	module_info1.hdr = (uint32_t)virt_to_phys(hdr_local);
	module_info1.len = len;
	
	count  = (len % 4096) ?  (len / 4096) + 1 : (len / 4096) ;
	
	//pfn = kmalloc(count * sizeof(unsigned long),GFP_KERNEL);
	pfn = alloc_page(GFP_KERNEL);
	pfn_array = (uint32_t *)(page_address(pfn));
	if(pfn == NULL)
		goto err2;

	module_info1.pfn = (uint32_t)page_to_phys(pfn);
	temp = module_info1.hdr;
	module_info1.count = count;
	
	for(i = 0; i<count; i++)
	 pfn_array[i] = ((temp + 4096*i) / 4096);
	 
	for(i = 0; i<count; i++)
		printk("pfn[%d] = %x \n", i, pfn_array[i]);
	
	memcpy(hdr_local,(void *)hdr,len);
	...
	//	if (1) {
		...
		// Fill the lkm_sec_information in stucture
		
		lkm_info1.buf = (unsigned long)virt_to_phys(buf);
		lkm_info1.len = buf_len;
		
	/*printk("sizeof(lkm_info1): %ld\n", sizeof(lkm_info1));	
	printk("sizeof(lkm_info1.buf): %ld\n", sizeof(lkm_info1.buf));	
	printk("sizeof(lkm_info1.len): %ld\n", sizeof(lkm_info1.len));	
	
	printk("sizeof(module_info1): %ld\n", sizeof(module_info1));	
	printk("sizeof(module_info1.hdr): %ld\n", sizeof(module_info1.hdr));	
	printk("sizeof(module_info1.len): %ld\n", sizeof(module_info1.len));	
	printk("sizeof(module_info1.pfn): %ld\n", sizeof(module_info1.pfn));	
	printk("sizeof(module_info1.count): %ld\n", sizeof(module_info1.count));
	printk("module_info1.hdr: %d, module_info1.len: %d, module_info1.pfn: %d, module_info1.count: %d\n", module_info1.hdr, module_info1.len, module_info1.pfn, module_info1.count);	
	
	printk("file len and ko len = %d  %d %x  count  = %x module_info1.pfn = %x\n",buf_len,len,hdr_local[0],count,module_info1.pfn);
	printk("file len and ko len = %x  %x %x\n",hdr_local[56576],hdr_local[56577],hdr_local[56578]);
	printk("file len and ko len = %x  %x %x\n",hdr_local[100412],hdr_local[100422],hdr_local[100423]);*/
	
	flush_cache_all();
	
	reg2 = (unsigned long)virt_to_phys(&lkm_info1);
	reg3 = (unsigned long)virt_to_phys(&module_info1);
	
	printk("[DEBUG] SMC cal : r0:0x%02lx\t r1:0x%02lx\t r2:0x%02lx\t r3:0x%02lx\n", reg0, reg1, reg2, reg3);
	
	ret = smc_call(reg0, reg1, reg2, reg3);
	
	printk("[DEBUG] SMC returnMain : r0:0x%02lx\t r1:0x%02lx\t r2:0x%02lx\t r3:0x%02lx\n", reg0, reg1, reg2, reg3);
